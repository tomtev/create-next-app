...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { walletAddress, prefix, pageSlug } = req.body;
    ...
    // If user has sufficient tokens, list the private content
    // Use the page slug as a prefix to only show content for this specific page
    const blobPrefix = prefix ? `${pageSlug}/${prefix}` : pageSlug;
    ...
    console.error('Error listing private content:', error);
    return res.status(500).json({ 
      error: 'Failed to list private content',
